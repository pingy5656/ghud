...
function PANEL:Init()
	...
	self.TextArea:SetFont( 'glorioushud.font.pickup15' )
	self.TextArea:SetTextColor( glorioushud.settings.vguiclrs.textclr ) 
	...
	self.Slider = self:Add( "GH_DSlider", self )
	...
	self.Slider.Paint = function( self, w, h )
		...
		local bx = self.Knob:GetPos()
		draw.RoundedBox( 5, 0, h / 2 - 1, w - 2, 5, glorioushud.settings.vguiclrs.sliderright )
		draw.RoundedBox( 5, 0, h / 2 - 1, bx, 5, glorioushud.settings.vguiclrs.sliderleft )

	end

	self.Label = vgui.Create ( "DLabel", self )
	self.Label:DockMargin( 0, 0, 0, -3 )
	self.Label:Dock( LEFT )
	self.Label:SetFont( 'glorioushud.font.pickup15' )
	self.Label:SetTextColor( glorioushud.settings.vguiclrs.textclr ) 
...
derma.DefineControl( "GH_DNumSlider", "Menu Option Line", table.Copy( PANEL ), "Panel" )
...
-- Compat
derma.DefineControl( "GH_Slider", "Backwards Compatibility", PANEL, "Panel" )
